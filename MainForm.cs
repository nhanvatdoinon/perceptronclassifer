...
namespace Classifier
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
		...
		/// Giải phóng các tài nguyên không được quản lý được sử dụng bởi Control và các kiểm soát con của nó và tùy chọn giải phóng các tài nguyên được quản lý.
		/// </summary>
		...
		//Cập nhật an toàn chuỗi thuộc tính văn bản của điều khiển
		...
		//Xóa an toàn chuỗi chế độ xem danh sách
		...
		//An toàn và nhanh chóng thêm mục vào list view trong win form
		...
		//Chủ đề an toàn thêm subitem vào một điều khiển ListView
		...
		//Khi nhấp vào nút "Tải" - tải dữ liệu
		private void loadButton_Click( object sender, System.EventArgs e )
		{
			// data file format: định dạng file 
			// X1, X2, ... Xn, class (0|1)

			// show file selection dialog: hiển thị hộp thoại chọn tệp
			if ( openFileDialog.ShowDialog( ) == DialogResult.OK )
			{
				StreamReader reader = null;

				// temp buffers (for 50 samples only):bộ tạm thời chỉ 50 mẫu
                float[,]	tempData = null;
				...
				try
				{
					...
					// read the data : đọc dữ liệu
					while ( ( samples < 50 ) && ( ( str = reader.ReadLine( ) ) != null ) )
					{
						// split the string : chia chuỗi 
						string[] strs = str.Split( ';' );
						if ( strs.Length == 1 )
							strs = str.Split( ',' );

						// allocate data array : phân bổ dữ liệu
						if ( samples == 0 )
						{
							variables = strs.Length - 1;//biến = độ dài chuỗi -1
                            tempData = new float[50, variables];//khởi tạo mảng 2 chiều
						}

						// parse data : phân tích dữ liệu
						...
						// search for min value:tìm kiếm giá trị tối thiệu
					...
					// allocate and set data:phân bổ và thiết lập dữ liệu
					data = new double[samples, variables];// mảng 2 chiều mẫu và biến
					Array.Copy( tempData, 0, data, 0, samples * variables );
					classes = new int[samples];//mảng 1 chiều
				...
				// show chart or not: hiển thị biểu đồ 
				bool showChart = ( variables == 2 );

				if ( showChart )
				{
					chart.RangeX = new Range( minX, maxX );//phụ thuộc vào minX và maxX
                    ShowTrainingData( ); //Hiển thị dữ liệu đào tạo trên biểu đồ
				}
		...
		// Update settings controls : cập nhật điều khiển cài đặt
		private void UpdateSettings( )
		{
			learningRateBox.Text = learningRate.ToString( );
		}

		// Update data in list view : cập nhật dữ liệu trong list view
		private void UpdateDataListView( )
		{
			// remove all curent data and columns 
			dataList.Items.Clear( );//xóa dữ liệu ở datalist
			dataList.Columns.Clear( );//xóa cột ở  datalist

			// add columns : thêm cột
		...
		// Show training data on chart:Hiển thị dữ liệu đào tạo trên biểu đồ
		private void ShowTrainingData( )
		{
			int class1Size = 0;
			int class2Size = 0;

			// calculate number of samples in each class	: tính số sample trong lớp
		...
		// Delegates to enable async calls for setting controls properties
		//Đại diện để bật lệnh gọi không đồng bộ để cài đặt thuộc tính điều khiển
		private delegate void EnableCallback( bool enable );

		// Enable/disale controls (safe for threading)
		//Bật / tắt điều khiển (an toàn cho luồng)
		...
		// Bật nút "Bắt đầu" - bắt đầu quy trình học
		private void startButton_Click(object sender, System.EventArgs e)
		{
			// get learning rate : tỉ lệ học tập 
			try
			{
			...
			// disable all settings controls : tắt điều khiển cài đặt
		...
		//Bật nút "Dừng" - dừng quy trình học
		...
		void SearchSolution( )
		{
			// prepare learning data : chuẩn bị dữ liệu học tập
			...
			// create perceptron : tạo perceptron
			ActivationNetwork	network = new ActivationNetwork( new ThresholdFunction( ), variables, 1 );//hàm bool ngưỡng là 1
